package com.myspring.pro29.ex04;
...
public class BoardController {
	...
	public ResponseEntity<List<MemberVO>> listArticles() {
		...
		// MemberVO 타입으로만 박스에 담아서 전달 해 주세요 (강력요구)
		for (int i = 0; i < 10; i++) {
	...
	// 클라이언트 주소 : /pro29/boards/{memberNO}
	@RequestMapping(value = "/{memberNO}", method = RequestMethod.GET)
	//@PathVariable("memberNO") Integer memberNO : 클라이언트 주소 요청 시 주소 뒤에 매개변수처럼 전달, 서버에서 해당 매개변수를 이용가능
	public ResponseEntity<MemberVO> findArticle (@PathVariable("memberNO") Integer memberNO) {
	...
	public ResponseEntity<String> addArticle (@RequestBody MemberVO MemberVO) {
	...
	//수정확인
	//@PathVariable("memberNO") Integer memberNO : 주소의 매개변수를 서버에서 가져와
	
	// 클라이언트 주소 : /pro29/boards/0918, PUT(:전달방식)
	@RequestMapping(value = "/{memberNO}", method = RequestMethod.PUT)
	public ResponseEntity<String> modArticle (@PathVariable("memberNO") Integer memberNO, @RequestBody MemberVO MemberVO) {
		ResponseEntity<String>  resEntity = null;
		try {
	...
	public ResponseEntity<String> removeArticle (@PathVariable("memberNO") Integer memberNO) {
		ResponseEntity<String>  resEntity = null;
		try {
